    ...
    <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px' }} className="page-padding">
      <header className="hero-padding" style={{
        ...
        <h1 style={{
          ...
          {cat.name_en}
          {cat.name_ko && <span style={{ color: 'var(--color-muted)', fontWeight: 400, marginLeft: '12px' }}>/ {cat.name_ko}</span>}
        </h1>
      </header>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 340px), 1fr))', gap: '40px' }} className="card-grid">
        {posts.map((post: any) => <PostCard key={post._id} post={post} />)}
      </div>
      {posts.length === 0 && (
        <p style={{ textAlign: 'center', padding: '80px', color: 'var(--color-muted)' }}>No posts yet.</p>
      )}
    </main>
  )
}
